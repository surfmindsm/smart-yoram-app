...
class NotificationSettingsService {
  ...
  static const String _keyChurchMessageNotifications = 'notification_church_message';
  ...
  /// 교회 메시지 알림 설정 저장
  Future<void> setChurchMessageNotifications(bool enabled) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setBool(_keyChurchMessageNotifications, enabled);
      developer.log('교회 메시지 알림 설정 저장: $enabled', name: 'NOTIFICATION_SETTINGS');
    } catch (e) {
      developer.log('교회 메시지 알림 설정 저장 실패: $e', name: 'NOTIFICATION_SETTINGS_ERROR');
    }
  }
  ...
  /// 교회 메시지 알림 설정 로드 (기본값: true)
  Future<bool> getChurchMessageNotifications() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      return prefs.getBool(_keyChurchMessageNotifications) ?? true;
    } catch (e) {
      developer.log('교회 메시지 알림 설정 로드 실패: $e', name: 'NOTIFICATION_SETTINGS_ERROR');
      return true;
    }
  }
  ...
  Future<bool> shouldShowNotification(String? notificationType) async {
    ...
    try {
      ...
      switch (notificationType) {
        ...
        case 'custom':
        case 'custom_message':
          return await getChurchMessageNotifications();
        default:
  ...
  Future<void> resetAllSettings() async {
    try {
      ...
      await prefs.remove(_keyChurchMessageNotifications);
